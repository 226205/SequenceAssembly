...
class KnownBruteForceTest {
	...
	void testDirect() {
		FileReader f_reader = new FileReader("src/unique_puzzle.txt");
		KnownBruteForce kbf_solution = new KnownBruteForce(f_reader.getPieceList(), f_reader.getXAxissAmount(), f_reader.getYAxissAmount(), f_reader.getSolutionTable(), f_reader.getDimensionAmount(), "unique_puzzle");
		
		Assertions.assertArrayEquals(kbf_solution.getSolvedTable(), f_reader.getSolutionTable());
	}
